			...
			t.Run(mode+"/checked="+strconv.FormatBool(checked), func(t *testing.T) {
				...
				for _, method := range []string{http.MethodGet, http.MethodPost} {
					...
					if method == http.MethodGet {
						...
						history := body.Data["recent_checkins"].([]any)
						require.Equal(t, record.Reward, history[0].(map[string]any)["reward"])
					} else {
						require.Equal(t, record.Reward, body.Data["record"].(map[string]any)["reward"])
