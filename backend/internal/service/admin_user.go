...
func (s *adminServiceImpl) getAllUserBalanceHistory(ctx context.Context, userID int64, params pagination.PaginationParams) ([]RedeemCode, int64, float64, error) {
	...
	codes := mergeBalanceHistoryCodes(redeemCodes, affiliateCodes, checkInCodes, params)
...
func (s *adminServiceImpl) listCheckInBalanceHistoryForMerge(ctx context.Context, userID int64, needed int) ([]RedeemCode, int64, error) {
	...
	for page := 1; len(out) < needed; page++ {
		params := pagination.PaginationParams{Page: page, PageSize: 1000}
		codes, currentTotal, err := s.listCheckInBalanceHistory(ctx, userID, params)
		if err != nil {
			return nil, 0, err
		}
		total = currentTotal
...
LIMIT $3`, userID, params.Offset(), params.Limit())
	...
	total, err := countCheckInBalanceHistory(ctx, s.entClient, userID)
	if err != nil {
		return nil, 0, err
	}
	return codes, total, nil
...
func countCheckInBalanceHistory(ctx context.Context, client *dbent.Client, userID int64) (int64, error) {
	rows, err := client.QueryContext(ctx, `
SELECT COUNT(*)
FROM user_checkins
...
func mergeBalanceHistoryCodes(redeemCodes, affiliateCodes, checkInCodes []RedeemCode, params pagination.PaginationParams) []RedeemCode {
