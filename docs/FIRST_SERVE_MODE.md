...
2. 新建或编辑目标 OpenAI 账号，在 **临时不可调度** 下方打开 **首服模式** 开关。保留已绑定的代理组；未绑定时，默认选择列表中第一个启用且至少有两个可用代理的组，并沿用默认的“全组可用代理”。已有指定代理配置时，仅从包含所有指定代理的组中选择。没有合适的组时展开配置并提示添加或启用代理组。可展开 **首服模式配置** 调整代理组、参数和允许的代理后保存；编辑页展开后在同一区域显示运行状态。
...
4. 使用此账号发起流式请求。账号编辑页展开配置后会显示组合，标注 HTTP / SSE 或 WebSocket，以及代理、组合 ID、首 token 延迟、到期时间和切换原因，每 10 秒刷新。收起配置或关闭弹窗后停止刷新。非流式、压缩请求不展示在首服状态中。

## 账号级配置

配置保存在 `accounts.extra.openai_first_serve`，不需要数据库迁移。缺少配置或 `reuse_scope` 时默认同账号共享组合；已显式保存按会话复用的账号保持原设置。首服开关默认关闭，开启后显示可展开的配置区域；已有首服账号会显示为开启。开启时隐藏账号下方的普通代理设置，统一在首服配置中选择代理组和允许的代理 / IP；关闭后恢复显示普通代理设置。

| 配置项 | 默认值 | 可配置范围 |
| --- | --- | --- |
| HTTP 组合复用范围 | 同账号所有会话共享 | 按会话独立 / 同账号所有会话共享 |
...
开启首服模式后，**HTTP 组合复用范围** 默认选择 **同账号所有会话共享组合**。API 字段为 `reuse_scope: "account"`；缺少该字段时也按账号共享。已显式保存 `reuse_scope: "session"` 的账号继续按会话独立复用，可在 **账号编辑 → 首服模式配置 → HTTP 组合复用范围** 中切换并保存。仅影响首服账号的 HTTP 请求，WebSocket 连接与历史继续独立管理。
...
关闭账号的 **首服模式** 开关并保存，即停用该模式，HTTP 后续请求及新客户端连接使用普通模式。开关关闭后可设置普通 WS mode；本次编辑中临时打开再关闭首服会恢复原选择，读取已有首服账号时普通模式默认回到 **上下文池**。HTTP 参数保存后下次请求生效，已经建立的客户端 WebSocket 保留建立时的配置。参数或代理组变化后不会复用旧策略的连接；若客户端携带旧 `previous_response_id` 重连并提示连接不可用，需要新建会话或发送完整历史。运行状态提示使用每个会话实际生效的参数。账号配置无需迁移；使用记录标识通过上述自动迁移新增字段，无新增依赖。
