...
共享模式中，同一上游账号的不同 API key、用户分组、会话及无标识请求，共用当前进程中的一个路由 ID（`session_id` / Responses `prompt_cache_key`）和代理。30 分钟到期或首 token 超阈值时，依照原有阈值、冷却和白名单规则切换；其他上游账号保持独立。状态页展示一条共享组合及该组合已使用次数。修改配置后开始新的组合，旧诊断记录可能继续显示一段时间。
...
状态页现区分流式、非流式和压缩请求，展示最近完成请求的输入/输出 token 用量、总耗时和可用的上游请求 ID。非流式无首 token 计时及压缩成功不参与首 token 达标判断，也不会覆盖组合已有的健康计时。流式返回但未记录计时会明确提示“未记录首 token 耗时”；失败有独立提示。用量为网关已解析到的上游 usage，不能用缺少耗时判断没有返回内容。
