export default {
    accounts: {
      ...
      openai: {
        ...
        wsMode: 'WS mode',
        wsModeDesc:
          'Applies only to the current OpenAI account type. Select Off to disable WS. WebSocket connections use the selected method only when gateway.openai_ws.mode_router_v2_enabled=true; otherwise, they use the context pool. HTTP/SSE first serve does not require this switch.',
        wsModeOff: 'Off (off)',
        ...
        firstServeSettings: {
          ...
          hint: "Applies to this account’s HTTP/SSE and WebSocket sessions. HTTP uses saved settings on the next request; existing WebSocket sessions keep their settings. If continuation becomes unavailable, start a new conversation or send full history.",
          ttl_minutes: "Combination lifetime (minutes)",
          ttft_seconds: "First-token threshold (seconds)",
          max_switches: "Consecutive rotation limit",
          cooldown_seconds: "Cooldown (seconds)",
          range: "Integer, {min}–{max}",
          group: "Proxy group",
          chooseGroup: "Select a proxy group",
          proxies: "Allowed proxies / IPs",
          all: "All available group members",
          selected: "Selected proxies only",
          proxyHint: "Only members of the selected group are used, for both initial connections and rotations. Inactive or expired proxies cannot create new connections. Prepare at least two distinct exit IPs; a proxy address may differ from its exit IP.",
          exitIP: "Detected exit IP: {ip}",
          unavailable: "Currently unavailable",
          noMembers: "No proxies found. Select a group or add proxies in Proxy management, then refresh this page.",
          ...
          fewAvailable: "Account {name}: Only {count} proxies are available in this range; rotation may fail. Check the group status and add or enable at least two distinct exits.",
        },
        ...
        firstServeKinds: { stream: "Streaming", non_stream: "Non-streaming", compact: "Context compaction" },
        firstServeStatus: "First serve status (this gateway node, latest 20 sessions)",
        firstServeEmpty: "No sessions yet. Save first serve mode, make an HTTP streaming or WebSocket request using this account, and refresh. HTTP/SSE does not require global WebSocket settings. For multiple nodes, check the node serving the request.",
        firstServeMetrics: "First token: {latency}s · Expires: {expires} · Rotations: {rotations}",
        firstServeSessionMissing: "Account {name}: No stable session identity was provided, so this request uses its own combination. Select account sharing above, or send the same session_id header or prompt_cache_key on every turn.",
        firstServeEnded: "Session ended",
        firstServeLoadFailed: "Account {name}: Failed to load first serve status. Click Refresh to retry.",
        firstServeReasons: {
          non_stream: "Non-streaming request completed. First-token timing is not applicable and is excluded from latency decisions.",
          compact: "Context compaction completed and is excluded from first-token latency decisions.",
          ttft_unavailable: "Request completed without recorded first-token timing. This does not mean no tokens were returned; check usage below.",
          request_failed: "This request did not finish successfully. Check error logs using the upstream request ID and verify this account’s proxy before retrying.",

          no_token: "No first token was detected, so latency is not rated. Check the response or use streaming requests.",
          observing: "Observing first-token latency for the new combination.",
          ready: "Latency is acceptable; keeping this combination until expiry.",
          slow: "First token exceeded {threshold} seconds. Rotating safely on the next turn.",
          context_incomplete: "Rotation deferred because this conversation has incomplete context. Send full history or start a new conversation.",
          proxy_unavailable: "No alternative proxy in the allowed range is available, or selection failed. Keeping the current combination; check selected proxies. Retrying after {cooldown} seconds.",
          cooldown: "{switches} rotations have not improved latency. Cooling down for {cooldown} seconds; check proxies and upstream load.",
          connection_failed: "A request on this combination failed. Check this account’s proxy and upstream error, then retry.",
        },
        wsModeFirstServe: 'First serve',
        wsModeFirstServeHint: 'Supports HTTP/SSE streaming and WebSocket. Rotate the upstream session identity and proxy on the next request when first-token latency exceeds the threshold or the combination expires, keeping current output running. Bind at least two distinct proxy exits. Response IDs still advance each turn.',
        firstServeProxyRequired: 'Account {name}: First serve requires a proxy group. Select a group with at least two distinct exits.',
        wsModeCtxPoolHint:
      ...
      poolModeInfo:
        'When enabled, upstream 429/403/401 errors will auto-retry without marking the account as rate-limited or errored. Suitable for upstream pointing to another gptplusch instance.',
