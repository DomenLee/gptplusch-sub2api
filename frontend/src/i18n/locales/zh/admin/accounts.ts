export default {
    accounts: {
      ...
      openai: {
        ...
        firstServeSettings: {
          title: "首服模式配置",
          hint: "仅作用于当前账号的 WebSocket 会话。保存后用于新建会话；已有会话保留原参数。修改后若续接提示连接不可用，请新建会话或发送完整历史。",
        ...
        firstServeEmpty: "暂无会话记录。保存模式并通过 WebSocket 发起请求后刷新；全局需启用 WebSocket v2 和模式路由。",
        ...
        firstServeReasons: {
          ...
          connection_failed: "当前会话上游连接失败，请检查该账号的代理及 WebSocket 配置后重试。",
        },
        wsModeFirstServe: '首服模式',
        wsModeFirstServeHint: '仅用于客户端 WebSocket 会话。首 token 超过阈值或组合使用到期后，在下一轮安全切换代理和连接。请绑定至少两个不同出口的代理组；响应 ID 仍逐轮更新。',
