export default {
    accounts: {
      ...
      openai: {
        ...
        wsMode: 'WS mode',
        wsModeDesc:
          '仅对当前 OpenAI 账号类型生效。选择“关闭”可禁用 WS；WebSocket 连接需全局 gateway.openai_ws.mode_router_v2_enabled=true 才按所选方式连接（首服的 HTTP/SSE 不受此限制），未开启时统一使用上下文池。',
        wsModeOff: '关闭（off）',
        ...
        firstServeSettings: {
          ...
          title: "首服模式配置",
          hint: "作用于当前账号的 HTTP/SSE 和 WebSocket 会话。HTTP 下次请求使用新配置，已有 WebSocket 会话保留原参数。修改后若续接不可用，请新建会话或发送完整历史。",
          ttl_minutes: "组合使用时长（分钟）",
          ttft_seconds: "首 token 阈值（秒）",
          max_switches: "连续切换上限（次）",
          ...
          all: "全组可用代理",
          selected: "仅指定代理",
          proxyHint: "仅从所选代理组中选择。首次连接和后续切换都遵守此范围，停用或过期代理不参与新连接。请准备至少两个不同出口 IP；代理地址不一定等于出口 IP。",
          exitIP: "检测出口 IP：{ip}",
          unavailable: "当前不可用",
          noMembers: "暂无代理记录，请选择代理组或前往代理管理添加代理后刷新页面。",
          ...
          fewAvailable: "账号 {name}：当前范围内仅有 {count} 个可用代理，可能无法切换。请检查代理组状态并添加或启用至少两个不同出口的代理。",
        },
        ...
        firstServeKinds: { stream: "流式", non_stream: "非流式", compact: "上下文压缩" },
        firstServeStatus: "首服模式状态（当前网关节点，最近 20 条组合记录）",
        firstServeEmpty: "暂无会话记录。保存首服模式后，使用此账号发起 HTTP 流式或 WebSocket 请求并刷新。HTTP/SSE 无需开启全局 WebSocket；多节点部署请查看实际处理请求的节点。",
        firstServeMetrics: "首 token：{latency} 秒 · 组合到期：{expires} · 已切换：{rotations} 次",
        firstServeSessionMissing: "账号 {name}：未提供稳定会话标识，本次使用独立组合。可在上方改为“同账号所有会话共享组合”，或让客户端每轮携带相同的 session_id 请求头或 prompt_cache_key。",
        firstServeEnded: "会话已结束",
        firstServeLoadFailed: "账号 {name}：首服状态加载失败，请点击刷新重试。",
        firstServeReasons: {
          non_stream: "非流式请求已完成，首 token 耗时不适用；不参与首 token 阈值判断。",
          compact: "上下文压缩已完成，不参与首 token 阈值判断。",
          ttft_unavailable: "请求已完成，但未记录首 token 耗时；不代表未返回 token，请结合下方用量查看。",
          request_failed: "本次请求未正常完成，请按上游请求 ID 检查错误日志，并检查此账号的代理后重试。",

          no_token: "本次请求未识别到首 token，暂不判断组合达标；请检查响应或使用流式请求。",
          observing: "正在观察新组合的首 token 延迟。",
          ready: "组合达标，继续使用至到期。",
          slow: "首 token 超过 {threshold} 秒，下一轮安全切换组合。",
          context_incomplete: "当前会话上下文不足，已延后切换并保持续聊；请发送完整历史或开启新会话。",
          proxy_unavailable: "允许范围内没有可切换的代理或查询失败，暂时保留旧组合；请检查所选代理，{cooldown} 秒后重试。",
          cooldown: "连续切换 {switches} 次仍未达标，冷却 {cooldown} 秒；请检查代理线路和上游负载。",
          connection_failed: "当前组合出现请求失败，请检查该账号的代理和上游错误后重试。",
        },
        wsModeFirstServe: '首服模式',
        wsModeFirstServeHint: '支持 HTTP/SSE 流式及 WebSocket。首 token 超过阈值或组合到期后，在下一请求切换上游会话标识与代理，不中断当前输出。请绑定至少两个不同出口的代理；响应 ID 仍逐轮更新。',
        firstServeProxyRequired: '账号 {name}：首服模式需要代理组，请先选择至少包含两个不同出口的代理组。',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 gptplusch 实例的场景。',
