                  ...
                  <div v-if="form.github_oauth_enabled" class="mt-4 space-y-4">
                    <div class="rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600 dark:bg-dark-800 dark:text-gray-300">
                      <template v-if="isZhLocale">
                        在对应开发者设置中创建 OAuth App；Homepage URL 填站点域名，Authorization callback URL 填下面的后端回调地址。
                      </template>
                      <template v-else>
                        Create an OAuth App in the provider developer settings. Use your site origin as Homepage URL and the backend callback URL below as Authorization callback URL.
                      </template>
                    </div>
              ...
              <template v-if="form.payment_enabled">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  ...
                  <div>
                    ...
                    >
                      {{
                        (form.payment_product_name_prefix || "gptplusch") +
                        " 100 " +
...
.settings-tab-active::after {
  ...
  content: "";
  background: linear-gradient(90deg, #737373, #262626);
}
