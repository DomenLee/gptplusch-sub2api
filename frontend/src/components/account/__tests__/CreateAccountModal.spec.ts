...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it('creates an account with custom first serve settings and resets them when reopened', async () => {
    ...
    expect(wrapper.get('[data-testid="first-serve-toggle"]').attributes('aria-checked')).toBe('false')
    await wrapper.get('form#create-account-form input[type="password"]').setValue('test-api-key')
    ...
    expect(createAccountMock.mock.calls[0]?.[0]?.extra?.openai_first_serve).toEqual({ reuse_scope: 'account', ttl_minutes: 12, ttft_seconds: 6, max_switches: 3, cooldown_seconds: 60, proxy_mode: 'all', proxy_ids: [] })
